...
def automate(simulation_dir, output_dir, all_problems):
    """Main function to automate a collection of problems.

    This processess command line options and runs all tasks with a scheduler
    that is configured using the ``jobs.json`` file if it is present.
    ...
    all_problem_names = [c.__name__ for c in all_problems]
    parser.add_argument(
    ...
    def _select_problem_classes(problems, classes):
        if problems == 'all':
            return classes
        else:
    ...
    problem_classes = _select_problem_classes(args.problems, all_problems)

    task = RunAll(
        simulation_dir=simulation_dir, output_dir=output_dir,
