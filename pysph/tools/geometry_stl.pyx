...
def point_sign(x1, y1, x2, y2, x3, y3):
    return (x1 - x3) * (y2 - y3) - (x2 - x3) * (y1 - y3)


def in_triangle(px, py, vx1, vy1, vx2, vy2, vx3, vy3):
    b1 = point_sign(px, py, vx1, vy1, vx2, vy2) < 0
    b2 = point_sign(px, py, vx2, vy2, vx3, vy3) < 0
    b3 = point_sign(px, py, vx3, vy3, vx1, vy1) < 0

    return ((b1 == b2) and (b2 == b3))


def interp_2d(v0, v1, dx):
    l = norm(v0 - v1)
    p = np.linspace(0., 1., int(l / dx + 2)).reshape(-1, 1)
    return v0.reshape(1, -1) * p + v1.reshape(1, -1) * (1. - p)


def get_triangle_sides(triangle, dx):
    sides = np.vstack([interp_2d(triangle[0], triangle[1], dx),
                       interp_2d(triangle[1], triangle[2], dx),
                       interp_2d(triangle[2], triangle[0], dx)])
    return sides[:, 0], sides[:, 1], sides[:, 2]


def projection_parameters(x, a, b, p):
    """Parameters of projection of x on surface (sa + tb + p)
    """
    return np.dot(x - p, a), np.dot(x - p, b)


def fill_triangle(triangle, h=0.1):
    ...
    b /= norm(b)
    st = np.array([projection_parameters(triangle[0], a, b, p),
                   projection_parameters(triangle[1], a, b, p),
                   projection_parameters(triangle[2], a, b, p)])
    ...
    for i in range(len(s_mesh)):
        mask[i] = in_triangle(s_mesh[i], t_mesh[i],
                              st[0, 0], st[0, 1],
                              st[1, 0], st[1, 1],
                              st[2, 0], st[2, 1])
...
def get_neighbouring_particles(pa_src, pa_dst, radius_scale):
    """
    Parameters
    ----------
...
def get_stl_mesh(stl_fname, dx_triangle):
    m = mesh.Mesh.from_file(stl_fname)
    x_list, y_list, z_list = [], [], []
    for i in range(len(m.vectors)):
        try:
            x1, y1, z1 = fill_triangle(m.vectors[i], dx_triangle)
            x2, y2, z2 = get_triangle_sides(m.vectors[i], dx_triangle)
            x_list.append(np.r_[x1, x2])
            y_list.append(np.r_[y1, y2])
            z_list.append(np.r_[z1, z2])
        except Exception as e:
            # A problem when triangles with zero areas exist
            print(e)
            print("Ignoring zero area triangle in STL file")
            continue
...
def get_stl_surface(stl_fname, dx_sph, h_sph, radius_scale=1.0,
    ...
    x, y, z = get_stl_mesh(stl_fname, dx_triangle)
    ...
    x_grid, y_grid, z_grid = get_neighbouring_particles(pa_grid, pa_mesh,
                                                        radius_scale)
