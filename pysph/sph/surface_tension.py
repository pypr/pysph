...
class ColorGradientUsingNumberDensity(Equation):
    ...
    def loop(self, d_idx, s_idx, d_color, s_color, d_cx, d_cy, d_cz,
        ...
        Cab = (s_color[s_idx] - d_color[d_idx]) * psiab1
        
        # color gradient
        d_cx[d_idx] += Cab * DWIJ[0]
        d_cy[d_idx] += Cab * DWIJ[1]
        d_cz[d_idx] += Cab * DWIJ[2]

    def post_loop(self, d_idx, d_ddelta, d_cx, d_cy, d_cz,
                  d_nx, d_ny, d_nz):
...
class InterfaceCurvatureFromNumberDensity(Equation):
    ...
    def loop(self, d_idx, s_idx, d_kappa, d_nx, d_ny, d_nz, s_nx, s_ny, s_nz, 
        ...
        nijdotdwij = (d_ny[d_idx] - s_ny[s_idx]) * DWIJ[1]
        nijdotdwij = (d_nz[d_idx] - s_nz[s_idx]) * DWIJ[2]
...
class ShadlooYildizSurfaceTensionForce(Equation):
    ...
    def loop(self, d_idx, d_au, d_av, d_aw, d_kappa, 
             d_nx, d_ny, d_nz, d_m, d_ddelta):

        # acceleration term Eq. (7) in [SY11]
        tmp = self.sigma * d_kappa[d_idx] * d_ddelta[d_idx]
