...
class EvaluateUhat(Equation):
    ...
    def loop(self, d_idx, d_h, s_h, s_x, s_y, s_z, d_x, d_y, d_z, s_rho,
        s_m, s_idx, XIJ, DWIJ, WIJ, s_uhat, d_Buh, s_vhat, d_Bvh, s_what, d_Bwh):
...
class ExtrapolateUhat(Equation):
    ...
    def loop(self, d_idx, d_uhat, d_uho, d_vhat, d_vho, d_what, d_who, d_xn, d_yn, d_zn, d_disp):
...
class CopyUhatFromGhost(Equation):
    def initialize_pair(self, d_idx, d_uhat, s_uhat, d_vhat, s_vhat, d_what, s_what, d_xn, d_yn, d_zn):
        #projection = 0.0+ s_uhat[d_idx] * d_xn[d_idx]+ s_vhat[d_idx] * d_yn[d_idx]+ s_what[d_idx] * d_zn[d_idx]
        d_uhat[d_idx] = -1.0 *  s_uhat[d_idx] #- 2.0 * projection * d_xn[d_idx]
       
        d_vhat[d_idx] =  s_vhat[d_idx] #- 2.0 * projection * d_yn[d_idx]
       
        d_what[d_idx] =  s_what[d_idx] #- 2.0 * projection * d_zn[d_idx]
...
class EvaluateU(Equation):
    ...
    def loop(self, d_idx, d_h, s_h, s_x, s_y, s_z, d_x, d_y, d_z, s_rho,
        s_m, s_idx, XIJ, DWIJ, WIJ, s_u, d_Bu, s_v, d_Bv, s_w, d_Bw):
        Vj = s_m[s_idx] / s_rho[s_idx]
        uj = s_u[s_idx]
        vj = s_v[s_idx]
        wj = s_w[s_idx]
...
class ExtrapolateU(Equation):
    ...
    def loop(self, d_idx, d_u, d_uo, d_v, d_vo, d_w, d_wo, d_xn, d_yn, d_zn, d_disp):
...
class CopyUFromGhost(Equation):
    def initialize_pair(self, d_idx, d_u, s_u, d_v, s_v, d_w, s_w, d_xn, d_yn, d_zn):
        #projection = 0.0+ s_u[d_idx] * d_xn[d_idx]+ s_v[d_idx] * d_yn[d_idx]+ s_w[d_idx] * d_zn[d_idx]
        d_u[d_idx] = -1.0 *  s_u[d_idx] #- 2.0 * projection * d_xn[d_idx]
       
        d_v[d_idx] =  s_v[d_idx] #- 2.0 * projection * d_yn[d_idx]
       
        d_w[d_idx] =  s_w[d_idx] #- 2.0 * projection * d_zn[d_idx]
...
class EvaluateP(Equation):
    ...
    def loop(self, d_idx, d_h, s_h, s_x, s_y, s_z, d_x, d_y, d_z, s_rho,
        s_m, s_idx, XIJ, DWIJ, WIJ, s_p, d_Bp):
        Vj = s_m[s_idx] / s_rho[s_idx]
        pj = s_p[s_idx]
        i4 = declare('int')
        i4 = 4*d_idx
...
class ExtrapolateP(Equation):
    ...
    def loop(self, d_idx, d_p, d_po, d_xn, d_yn, d_zn, d_disp):
