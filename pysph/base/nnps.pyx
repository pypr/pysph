...
cdef class SpatialHashNNPS(NNPS):
    ...
    def __init__(self, int dim, list particles, double radius_scale = 2.0,
            ...
            bint sort_gids = False, LL_INT table_size = 131072):
...
cdef class ExtendedSpatialHashNNPS(NNPS):
    ...
    def __init__(self, int dim, list particles, double radius_scale = 2.0,
            ...
            LL_INT table_size = 131072, bint approximate = False):
        NNPS.__init__(
            self, dim, particles, radius_scale, ghost_layers, domain,
            cache, sort_gids
        )
    ...
    cdef inline int neighbor_boxes(self, int i, int j, int k,
        ...
        if self.approximate:
            mask_len = self.h_mask_approx(x_mask, y_mask, z_mask)
        else:
            mask_len = self.h_mask_exact(x_mask, y_mask, z_mask)
