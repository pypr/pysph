...
def generate_kernel_code(kernel_factory):
    ...
    cpdef kernel(xi, yi, zi, xj, yj, zj, h):
        return ${kernel_name}(xi, yi, zi, xj, yj, zj, h)
        
    cpdef gradient(xi, yi, zi, xj, yj, zj, h):
        cdef double[3] result
        ${gradient_name}(xi, yi, zi, xj, yj, zj, h, result)
        return result[0], result[1], result[2]
    """)
    kernel = kernel_factory()
    kernel_name = kernel.__class__.__name__ + 'Kernel'
    gradient_name = kernel.__class__.__name__ + 'Gradient'
    kernel_code = kernel.cython_code().get('helper')
