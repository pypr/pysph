...
class TestDeviceArray(TestCase):
    ...
    def test_copy(self):
        ...
        # Then
        assert np.all(dev_array.dev.get() == dev_array_copy.dev.get())

        dev_array_copy.dev[0] = 2
        assert dev_array.dev[0] != dev_array_copy.dev[0]
    ...
    def test_extend(self):
        ...
        array = Array(dev_ary=cl_array)

        # When
        dev_array.extend(array)

        # Then
        assert np.all(dev_array.dev[-len(cl_array)].get() == cl_array.get())
    ...
    def test_remove(self):
        ...
        indices = Array(dev_ary=cl.array.arange(get_queue(),
                        0, 8, 1, dtype=np.int32))

        # When
        dev_array.remove(indices)

        # Then
        assert np.all(dev_array.dev.get() == (8 + indices.dev).get())
    ...
    def test_align(self):
        ...
        dev_array.set_data(orig_array)
        indices = Array(dev_ary=cl.array.arange(get_queue(),
                        15, -1, -1, dtype=np.int32))

        # When
        dev_array.align(indices)

        # Then
        assert np.all(dev_array.dev.get() == indices.dev.get())
    ...
    def test_copy_values(self):
        ...
        dest = Array(dev_ary=cl.array.empty(get_queue(), 8, dtype=np.int32))
        indices = Array(dev_ary=cl.array.arange(get_queue(), 0, 8, 1, dtype=np.int32))

        # When
        dev_array.copy_values(indices, dest)

        # Then
        assert np.all(dev_array.dev[:len(indices.dev)].get() == dest.dev.get())

    def test_min_max(self):
        # Given
        dev_array = self.make_dev_array()
        dev_array.fill(2)
        dev_array.dev[0], dev_array.dev[1] = 1, 10
