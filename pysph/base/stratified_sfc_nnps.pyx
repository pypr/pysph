...
cdef class StratifiedSFCGPUNNPS(GPUNNPS):
    def __init__(self, int dim, list particles, double radius_scale=2.0,
        ...
        self.num_levels = num_levels
        self.interval_size = \
                (self.cell_size - self.radius_scale*self.hmin)/num_levels + EPS
    ...
    cpdef _refresh(self):
        ...
        self.cell_sizes = cl.array.zeros(self.queue, self.num_levels,
        ...
        for i from 0<=i<self.narrays:
            ...
            self.start_idx_levels.append(cl.array.zeros(self.queue,
        ...
        cdef int max_num_cells = (<int> ceil(max_length/self.hmin))**3

        self.max_num_bits = <int> ceil(log2(max_num_cells))
    ...
    cpdef _bin(self, int pa_index):
        ...
        fill_pids(pa_wrapper.gpu_x, pa_wrapper.gpu_y, pa_wrapper.gpu_z, pa_wrapper.gpu_h,
                ...
                self.pid_keys[pa_index], self.pids[pa_index], self.cell_sizes,
    ...
    cdef void find_neighbor_lengths(self, nbr_lengths):
        ...
        find_nbr_lengths(self.dst.gpu_x, self.dst.gpu_y, self.dst.gpu_z,
                ...
                self.radius_scale, self.cell_size, self.cell_sizes,
                self.start_idx_levels[self.src_index], self.max_num_bits)
    ...
    cdef void find_nearest_neighbors_gpu(self, nbrs, start_indices):
        ...
        find_nbrs(self.dst.gpu_x, self.dst.gpu_y, self.dst.gpu_z,
                ...
                self.src.get_number_of_particles(), self.pid_keys[self.src_index],
                self.pids[self.dst_index], self.pids[self.src_index],
                start_indices, nbrs, self.radius_scale, self.cell_size,
                self.cell_sizes, self.start_idx_levels[self.src_index],
                self.max_num_bits)
