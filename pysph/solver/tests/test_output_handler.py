...
class TestSolverUtils(TestCase):
    ...
    def test_dump_and_load_works_by_default(self):
        ...
        output_handler = OutputHandler(self.root)
        fname = 'simple'
        output_handler.dump(fname, [pa], solver_data={'dt': dt})
        data = output_handler.load(fname)
    ...
    def test_dump_and_load_works_with_numpy(self):
        ...
        dt = 1.0
        pa = get_particle_array(name='fluid', x=x, y=y)
        output_handler = OutputHandler(self.root, 'npz')
        fname = 'simple_numpy'
        output_handler.dump(fname, [pa], solver_data={'dt': dt})
        data = output_handler.load(fname)
        solver_data = data['solver_data']
    ...
    def test_dump_and_load_works_with_numpy_compression(self):
        x = np.linspace(0, 1.0, 10)
        y = x*2.0
        dt = 1.0
        pa = get_particle_array(name='fluid', x=x, y=y)
        output_handler = OutputHandler(self.root, 'npz', compress=True)
        fname = 'simple_numpy_compress'
        output_handler.dump(fname, [pa], solver_data={'dt': dt})
        data = output_handler.load(fname)
        solver_data = data['solver_data']
        arrays = data['arrays']
    ...
    def test_dump_and_load_with_partial_data_dump(self):
        ...
        output_handler = OutputHandler(self.root)
        fname = 'simple_partial'
        output_handler.dump(fname, [pa], solver_data={})
        data = output_handler.load(fname)
        solver_data = data['solver_data']
        arrays = data['arrays']
        pa1 = arrays['fluid']
