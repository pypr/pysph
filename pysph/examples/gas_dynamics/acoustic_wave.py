...
from pysph.sph.scheme import GSPHScheme, ADKEScheme, GasDScheme, SchemeChooser
...
class AcousticWave(Application):
    ...
    def create_domain(self):
        return DomainManager(xmin=0, xmax=1, periodic_in_x=True)
    ...
    def create_particles(self):
        ...
        rho = self.rho_0 + self.delta_rho * numpy.sin(self.k * x)
        p = self.p_0 + self.c_0 ** 2 * self.delta_rho * numpy.sin(self.k * x)
        u = self.c_0 * self.delta_rho * numpy.sin(self.k * x) / self.rho_0
    ...
    def create_scheme(self):
        gsph = GSPHScheme(fluids=['fluid'], solids=[], dim=self.dim,
                          gamma=self.gamma, kernel_factor=1.0, g1=0., g2=0.,
                          rsolver=7, interpolation=1, monotonicity=1,
                          interface_zero=True, hybrid=False, blend_alpha=5.0,
                          niter=40, tol=1e-6, has_ghosts=True)

        mpm = GasDScheme(fluids=['fluid'], solids=[], dim=self.dim,
                         gamma=self.gamma, kernel_factor=1.2, alpha1=0,
                         alpha2=0, beta=2.0, update_alpha1=False,
                         update_alpha2=False, has_ghosts=True)

        crksph = CRKSPHScheme(fluids=['fluid'], dim=self.dim, rho0=0, c0=0,
                              nu=0, h0=0, p0=0, gamma=self.gamma, cl=2,
                              has_ghosts=True)

        adke = ADKEScheme(fluids=['fluid'], solids=[], dim=self.dim,
                          gamma=self.gamma, alpha=0, beta=0.0, k=1.5, eps=0.0,
                          g1=0.0, g2=0.0, has_ghosts=True)

        psph = PSPHScheme(fluids=['fluid'], solids=[], dim=self.dim,
                          gamma=self.gamma, hfact=1.2)

        tsph = TSPHScheme(fluids=['fluid'], solids=[], dim=self.dim,
                          gamma=self.gamma, hfact=1.2)

        s = SchemeChooser(default='gsph', gsph=gsph, mpm=mpm, crksph=crksph,
                          adke=adke, psph=psph, tsph=tsph)
    ...
    def post_process(self):
        ...
        x_c = pa.x
        u = self.c_0 * self.delta_rho * numpy.sin(self.k * x_c) / self.rho_0
        u_c = pa.u
        ...
        rho = self.rho_0 + self.delta_rho * numpy.sin(self.k * x_c)
