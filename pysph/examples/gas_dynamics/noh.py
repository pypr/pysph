...
class NohImplosion(Application):
    def create_particles(self):
        x, y = numpy.mgrid[xmin:xmax:dx, ymin:ymax:dx]
        ...
        fluid = gpa(name='fluid', x=x, y=y, m=m, rho=rho, h=h, u=u, v=v,
                    p=1e-12, e=2.5e-11, h0=h.copy())
    ...
    def create_scheme(self):
        mpm = GasDScheme(fluids=['fluid'], solids=[], dim=dim, gamma=gamma,
                         ...
                         alpha2=alpha2, beta=beta, adaptive_h_scheme="mpm",
                         update_alpha1=True, update_alpha2=True,
                         has_ghosts=True)

        crksph = CRKSPHScheme(fluids=['fluid'], dim=2, rho0=0, c0=0, nu=0,
                              h0=0, p0=0, gamma=gamma, cl=2, has_ghosts=True)

        gsph = GSPHScheme(fluids=['fluid'], solids=[], dim=dim, gamma=gamma,
                          kernel_factor=1.5, g1=0.25, g2=0.5, rsolver=7,
        ...
        s = SchemeChooser(default='crksph', crksph=crksph, mpm=mpm, adke=adke,
