...
from pysph.sph.rigid_body import (
    BodyForce, NumberDensity, RigidBodyCollision, RigidBodyMoments,
    RigidBodyMotion, RK2StepRigidBody, ViscosityRigidBody, PressureRigidBody)
...
class RigidBlockInTank(Application):
    def create_particles(self):
        ...
        block = get_particle_array_rigid_body(name='block', x=x, y=y, h=h, m=m,
                                              rho=rho)
    ...
    def create_solver(self):
        ...
        solver = Solver(kernel=kernel, dim=2, integrator=integrator, tf=tf,
                        dt=dt, adaptive_timestep=False)
        return solver
    ...
    def create_equations(self):
        equations = [
            ...
            Group(
                equations=[
                    TaitEOS(dest='fluid', sources=None, rho0=rho0, c0=c0,
                            gamma=gamma),
                    TaitEOSHGCorrection(dest='solid', sources=None, rho0=rho0,
                                        c0=c0, gamma=gamma),
                    TaitEOSHGCorrection(dest='block', sources=None, rho0=rho0,
                                        c0=c0, gamma=gamma),
                ], ),
            ...
            Group(equations=[
                ...
                PressureRigidBody(dest='fluid', sources=['block', 'solid'],
                                  rho0=rho0),
                ViscosityRigidBody(dest='fluid', sources=['block', 'solid'],
                                   rho0=rho0, nu=nu),

                # Position step with XSPH
                XSPHCorrection(dest='fluid', sources=['fluid']),
                RigidBodyCollision(dest='block', sources=['solid'], k=1.0,
                                   d=2.0, eta=0.1, kt=0.1),
            ]),
